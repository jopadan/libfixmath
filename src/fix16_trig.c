...
/* At this point, retval equals sin(inAngle) on important points ( -PI, -PI/2,
   0, PI/2, PI), but is not very precise between these points
 */
#ifndef FIXMATH_FAST_SIN
...
fix16_t fix16_sin(fix16_t inAngle)
{
    ...
    if (tempAngle >= fix16_pi)
    {
        ...
        tempOut =
            -(tempAngle >= _fix16_sin_lut_count ? fix16_one
                                                : _fix16_sin_lut[tempAngle]);
    }
    else
    {
        if (tempAngle >= (fix16_pi >> 1))
...
#else // Fast implementation, runs at 159% the speed of above 'accurate' version
...
fix16_t fix16_atan2(fix16_t inY, fix16_t inX)
{
    ...
    if (inX >= 0)
    {
        ...
        angle =
            fix16_mul(0x00003240, r_3) - fix16_mul(0x0000FB50, r) + PI_DIV_4;
    }
    else
    {
