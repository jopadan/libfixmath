...
// By default, the output is divided by the transform length to get a normalized
// FFT. Input_convert determines the scaling of intermediate values.
// Multiplication by 256 gives a nice compromise between precision and numeric
// range.
#ifndef INPUT_TYPE
...
// Mix N blocksize-sized transforms pairwise together to get N/2
// 2*blocksize-sized transforms.
                      ...
                      unsigned blockpairs)
{
    ...
    for (i = 0; i < blocksize; i++)
    {
        ...
        for (j = 0; j < blockpairs; j++)
        {
            ...
            fix16_t re =
                fix16_mul(rp[blocksize], c) - fix16_mul(ip[blocksize], s);
            fix16_t im =
                fix16_mul(ip[blocksize], c) + fix16_mul(rp[blocksize], s);
...
// arrays. Size of each array is the same as transform_length. Transform length
// must be a power of two and atleast 4.
void fix16_fft(INPUT_TYPE* input, fix16_t* real, fix16_t* imag,
               unsigned transform_length)
{
    ...
    {
        four_point_dft(input + INPUT_INDEX(rbit_n(i, log_length - 2)),
