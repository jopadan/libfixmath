...
function(create_variant name)
    add_library(fixmath_${name} STATIC ${libfixmath-srcs})
    target_include_directories(fixmath_${name} INTERFACE
        ${CMAKE_CURRENT_SOURCE_DIR})
    target_compile_definitions(fixmath_${name} PRIVATE PREFIX=${name} ${ARGN})
    target_compile_options(fixmath_${name} PRIVATE ${sanitizer_opts})
    target_link_options(fixmath_${name} PRIVATE ${sanitizer_opts})
    add_executable(tests_${name} ${tests-srcs})
    target_link_libraries(tests_${name} PRIVATE fixmath_${name} m)
